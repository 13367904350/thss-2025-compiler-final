...
def generate_llvmir(sysy_file):
    test_dir = Path("./test/resources/float")
    if not test_dir.exists():
        return False, "Float test directory not found."
...
def main():    
    subprocess.run("rm -f ./test/resources/float/*.ll", shell=True)
    subprocess.run("rm -f ./test/resources/float/*.output", shell=True)
