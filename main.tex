...
\twocolumn[
    \begin{@twocolumnfalse}
        ...
        \begin{abstract}
            ...
            本报告总结了我们小组在 SysY 编译器中增加浮点数、指针、For循环与结构体支持的实现过程。针对浮点数，我们重点解决了整型与浮点数的混合运算、自动类型转换以及在 \texttt{if/while} 条件中的布尔判断问题。针对指针，我们实现了取地址（$\&$）与解引用（$*$）操作，并处理了指针与数组在函数传参时的退化问题。针对 For 循环，我们构建了包含四种基本块的标准控制流结构，并正确处理了 \texttt{break} 与 \texttt{continue}。针对结构体，我们实现了类型的定义与注册，以及基于偏移量的成员访问机制。通过对类型系统和 IR 生成逻辑的重构，编译器现在能够正确处理这些高级语言特性，并顺利通过了所有测试用例。
            \par\vspace{1.5em}
        \end{abstract}
    \end{@twocolumnfalse}
]
...
%===========================================================
\section{引言}
本研究的工作重心是在基础的 SysY 编译器上，通过重构底层架构，实现了对 IEEE 754 浮点数、多级指针、For 循环以及结构体的支持。我们并没有简单地添加几种新类型，而是深入调整了从语法解析到中间代码（IR）生成的整个流程。
具体的工作主要集中在以下四个方面。
...
在 For 循环实现方面：
\begin{enumerate}
    \item 控制流构建：设计了完整的 BasicBlock 结构（cond, body, inc, end），确保初始化、条件检查、循环体与步进逻辑的正确串联。
    \item 跳转控制：通过维护循环栈结构，正确支持了 break（跳至 end）与 continue（跳至 inc）语义。
\end{enumerate}

在结构体实现方面：
\begin{enumerate}
    \item 类型扩展：在符号表中引入了结构体类型描述，支持成员变量的类型记录与索引映射。
    \item 成员访问：实现了结构体成员访问符 . 的翻译，通过 GEP 指令精确计算成员地址偏移。
\end{enumerate}
...
%===========================================================
\section{结论}
通过本次期末作业的拓展研究，我们成功为 SysY 编译器补齐了浮点数、指针、For 循环以及结构体这四大核心功能。在实现过程中，我们不仅掌握了如何利用 LLVM IR 指令（如 \texttt{fcmp}, \texttt{getelementptr}, \texttt{phi}）来表达高级语言语义，更重要的是理清了内存地址计算、类型系统扩展与控制流图构建之间的底层联系。

目前，编译器已经能够稳定处理混合类型运算、多层指针嵌套访问、复杂的循环控制流以及结构化的数据组织。虽然在指针的安全检查（如野指针检测）方面还有提升空间，但就功能完整性而言，已经达到了我们的预期要求，能够支撑起包括复杂算法与数据结构在内的各类应用程序。
%===========================================================
% 附录区域
%===========================================================
\clearpage % 强制换页
